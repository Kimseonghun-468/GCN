    ...
    def __init__(self, Ls, F, K, p, M, M1, filter='chebyshev5', brelu='b1relu', pool='mpool1',
        ...
        if Nfc_shared > 0:
            ...
            for i in range(Nfc_shared):
                ...
                print('    representation: M_{} = {}'.format(i+1, M1[i]))
                M_last = M1[i-1] if i > 0 else len(self.Ls)
                print('    weights: M_{} * M_{} = {} * {} = {}'.format(
                        i, i+1, M_last, M1[i], M_last*M1[i]))
                print('    biases: M_{} = {}'.format(i+1, M1[i]))
            print('--------------------------------------')
...
#                        print('  filter:')
                        x = self.filter(x, self.Ls[kegg_id][i], self.F[i], self.K[i])
#                        print(x.shape)
...
#                print(x.shape)
        ...
        if len(self.M1)>0:
            ...
            for i,M in enumerate(self.M1[:-1]):
...
#                print('concat_logits')
                x_concat = self.fc(x_concat, self.M1[-1], relu=True)
#                print(x.shape)
        return x_concat
