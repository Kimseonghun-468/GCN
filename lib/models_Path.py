...
class base_model(object):
    ...
    def predict(self, dataset, labels=None, sess=None):
        loss = 0
        ...
        for begin in range(0, size, self.batch_size):
            ...
            for i, data in enumerate(dataset):
                ...
                feed_dict[self.ph_data[i]] = batch_data
...
#             feed_dict = {self.ph_data: batch_data, self.ph_dropout: 1}
    ...
    def evaluate(self, dataset, labels, sess=None):
        """
        ...
        predictions, loss = self.predict(dataset, labels, sess)
        # modified
#         print(labels)
#         print(predictions)
...
#         f1 = 0.5
    ...
    def fit(self, train_dataset, train_labels, val_dataset, val_labels):
        t_process, t_wall = time.process_time(), time.time()
        ...
        num_steps = int(self.num_epochs * train_dataset[0].shape[0] / self.batch_size)
        for step in range(1, num_steps+1):

            # Be sure to have used all the samples before using one a second time.
            if len(indices) < self.batch_size:
                indices.extend(np.random.permutation(train_dataset[0].shape[0]))
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                # modified
                string, loss = self.evaluate(val_dataset, val_labels, sess)
    ...
    def build_graph(self, M_0s):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                self.ph_data = []
                for i in range(len(M_0s)):
                    self.ph_data.append(tf.placeholder(tf.float32, (self.batch_size, M_0s[i]), 'data'))
...
#             print(op_logits.shape, self.ph_labels.shape)
...
class cgcnn(base_model):
    ...
    """
    def __init__(self,Ls, F, K, p, M, M1, filter='chebyshev5', brelu='b1relu', pool='mpool1',
        ...
        self.M_0s = []
        print('--------------------------------------!!')
        for kegg_id, L in enumerate(Ls):
            print('---{}_th KEGG GCN ---'.format(kegg_id))
            # Verify the consistency w.r.t. the number of layers.
            assert len(L) >= len(F[kegg_id]) == len(K[kegg_id]) == len(p[kegg_id])
            assert np.all(np.array(p[kegg_id]) >= 1)
            p_log2 = np.where(np.array(p[kegg_id]) > 1, np.log2(p[kegg_id]), 0)
            assert np.all(np.mod(p_log2, 1) == 0)  # Powers of 2.
            assert len(L) >= 1 + np.sum(p_log2)  # Enough coarsening levels for pool sizes.

            # Keep the useful Laplacians only. May be zero.
            M_0 = L[0].shape[0]
            self.M_0s.append(M_0)
            j = 0
            self.L = []
            for pp in p[kegg_id]:
                self.L.append(L[j])
            ...
            Ngconv = len(p[kegg_id])
            Nfc = len(M[kegg_id])
            print('  input: M_0 = {}'.format(M_0))
            for i in range(Ngconv):
                print('  layer {0}: cgconv{0}'.format(i+1))
                print('    representation: M_{0} * F_{1} / p_{1} = {2} * {3} / {4} = {5}'.format(
                        i, i+1, L[i].shape[0], F[kegg_id][i], p[kegg_id][i], L[i].shape[0]*F[kegg_id][i]//p[kegg_id][i]))
                F_last = F[i-1] if i > 0 else 1
                print('    weights: F_{0} * F_{1} * K_{1} = {2} * {3} * {4} = {5}'.format(
                        i, i+1, F_last, F[kegg_id][i], K[kegg_id][i], F_last*F[kegg_id][i]*K[kegg_id][i]))
                if brelu == 'b1relu':
                    print('    biases: F_{} = {}'.format(i+1, F[kegg_id][i]))
                elif brelu == 'b2relu':
                    print('    biases: M_{0} * F_{0} = {1} * {2} = {3}'.format(
                            i+1, L[i].shape[0], F[kegg_id][i], L[kegg_id][i].shape[0]*F[kegg_id][i]))
            for i in range(Nfc):
                name = 'logits (softmax)' if i == Nfc-1 else 'fc{}'.format(i+1)
                print('  layer {}: {}'.format(Ngconv+i+1, name))
                print('    representation: M_{} = {}'.format(Ngconv+i+1, M[kegg_id][i]))
                M_last = M[kegg_id][i-1] if i > 0 else M_0 if Ngconv == 0 else L[-1].shape[0] * F[kegg_id][-1] // p[kegg_id][-1]
                print('    weights: M_{} * M_{} = {} * {} = {}'.format(
                        Ngconv+i, Ngconv+i+1, M_last, M[kegg_id][i], M_last*M[kegg_id][i]))
                print('    biases: M_{} = {}'.format(Ngconv+i+1, M[kegg_id][i]))
        print('--------------------------------------')
        Nfc_shared = len(M1[kegg_id])
        if Nfc_shared > 0:
            print('concat representation: M_0={}'.format(len(self.Ls)))
            print('shared FCNN architecture')
            for i in range(Nfc_shared):
                name = 'logits (softmax)' if i == Nfc_shared-1 else 'fc{}'.format(i+1)
                print('  layer {}: {}'.format(i+1, name))
                print('    representation: M_{} = {}'.format(i+1, M1[kegg_id][i]))
                M_last = M1[kegg_id][i-1] if i > 0 else len(self.Ls)
                print('    weights: M_{} * M_{} = {} * {} = {}'.format(
                        i, i+1, M_last, M1[kegg_id][i], M_last*M1[kegg_id][i]))
                print('    biases: M_{} = {}'.format(i+1, M1[kegg_id][i]))
            print('--------------------------------------')

        # Store attributes and bind operations.
        self.F, self.K, self.p, self.M, self.M1 = F, K, p, M, M1
    ...
    def _inference(self, xs, dropout):
        # Graph convolutional layers.
        x_concat = []
        for kegg_id, x in enumerate(xs):
            x = tf.expand_dims(x, 2)  # N x M x F=1
#            print('--> input x dimension')
#            print(x.shape)
            for i in range(len(self.p[kegg_id])):
                with tf.variable_scope('kegg{}_conv{}'.format(kegg_id, i+1)):
                    with tf.name_scope('filter'):
#                        print('kegg{}_conv{}'.format(kegg_id, i+1))
#                        print('  filter:')
                        x = self.filter(x, self.Ls[kegg_id][i], self.F[kegg_id][i], self.K[kegg_id][i])
#                        print(x.shape)
...
#                        print('  pooling:')
                        x = self.pool(x, self.p[kegg_id][i])
#                        print(x.shape)
                        
            # Fully connected hidden layers.
            N, M, F = x.get_shape()
            x = tf.reshape(x, [int(N), int(M*F)])  # N x M
            for i,M in enumerate(self.M[kegg_id][:-1]):
                with tf.variable_scope('kegg{}_fc{}'.format(kegg_id, i+1)):
#                    print('kegg{}_fc{}'.format(kegg_id, i+1))
                    x = self.fc(x, M)
                    x = tf.nn.dropout(x, dropout)
#                    print(x.shape)

            # Logits linear layer, i.e. softmax without normalization.
            with tf.variable_scope('kegg{}_logits'.format(kegg_id)):
#                print('kegg{}_logits'.format(kegg_id))
                x = self.fc(x, self.M[kegg_id][-1], relu=True)
                x_concat.append(x)
#                print(x.shape)
                
        if len(self.M1[kegg_id])>0:
            x_concat = tf.concat(x_concat, axis=1)
            for i,M in enumerate(self.M1[kegg_id][:-1]):
                with tf.variable_scope('fc{}'.format(i+1)):
#                    print('concat_fc{}'.format(i+1))
                    x_concat = self.fc(x_concat, M)
                    x_concat = tf.nn.dropout(x_concat, dropout)
#                    print(x.shape)
            with tf.variable_scope('logits'):
#                print('concat_logits')
                x_concat = self.fc(x_concat, self.M1[kegg_id][-1], relu=True)
#                print(x.shape)
        return x_concat
